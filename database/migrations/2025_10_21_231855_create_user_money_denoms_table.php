...
return new class extends Migration
{
    ...
    {
        Schema::create('user_money_denoms', function (Blueprint $table) {
            $table->id();
            $table->foreignId('fujimien_user_id')->comment('ふじみ園利用者ID')->constrained('fujimiens');
            $table->foreignId('business_id')->comment('事業所ID')->constrained('businesses');
            $table->string('money_denomination_name', 255)->nullable()->comment('金種名');
            $table->date('start_date')->nullable()->comment('開始日');
            $table->date('end_date')->nullable()->comment('終了日');
            $table->string('maximum_money_denomination')->nullable()->comment('最大金種');
    ...
    {
        Schema::dropIfExists('user_money_denoms');
    }
};
